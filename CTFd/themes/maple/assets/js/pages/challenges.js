...
function markSolves() {
  challenges.map((challenge) => {
    if (challenge.solved_by_me) {
      ...
      btn.prepend(
        '<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style="height: 24px;width: 24px;color:white"> <path d="M15 6h2v2h-2V6zm-2 4V8h2v2h-2zm-2 2v-2h2v2h-2zm-2 2v-2h2v2H9zm-2 2v-2h2v2H7zm-2 0h2v2H5v-2zm-2-2h2v2H3v-2zm0 0H1v-2h2v2zm8 2h2v2h-2v-2zm4-2v2h-2v-2h2zm2-2v2h-2v-2h2zm2-2v2h-2v-2h2zm2-2h-2v2h2V8zm0 0h2V6h-2v2z" fill="currentColor"/> </svg>'
      );
    }
  });
}
  ...
  return CTFd.api.get_challenge_list().then(function(response) {
    ...
    for (let i = 0; i <= challenges.length - 1; i++) {
      ...
      if (solves.indexOf(chalinfo.id) == -1) {
        chalbutton = $(
          "<button class='rpgui-container framed-grey challenge-button w-100 text-truncate pt-3 pb-3 mb-2' value='{0}'></button>".format(
            chalinfo.id
          )
        );
      } else {
