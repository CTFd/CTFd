...
* Fix subdirectory deployments in a generic manner by modifying `request.path` to combine both `request.script_root` and `request.path`.
    * Also creating a request preprocessor to redirect users into the true CTFd app when deploying on a subdirectory.
...
* Add `account_url` field to the response of `/api/v1/<challenge_id>/solves` so the client knows where to redirect users.
