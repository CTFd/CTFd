...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        for challenge in old_data['challenges']:
            ...
            db.session.add(Challenges(**challenge))
            db.session.commit()
        ...
        for submission in submissions:
            ...
            if model == 'solve':
                ...
                db.session.add(Solves(**submission))
                db.session.commit()
            elif model == 'wrong_key':
                # new_conn['submissions'].insert(dict(submission))
                db.session.add(Fails(**submission))
                db.session.commit()
        del old_data['solves']
        ...
        for tracking in old_data['tracking']:
            ...
            db.session.add(Tracking(**tracking))
            db.session.commit()
